...
########## CUSTOM LAYERS ##########
# parameter suffix: _i (int and boolean), _f (float), _s (string), _t (tensor), _g (sub-graph)
########## CUSTOM LAYERS ##########
...
# So ONNX and pytorch carry different parameters,
# but caffe and pytorch seems to be carry same parameters,
# so we skip the exception handling here, simply ignore and pass on the parameter values.
...
#加上ceil_mode参数
def avg_pool1d(g, input, kernel_size, stride, padding, ceil_mode, count_include_pad):
    if ceil_mode:
    ...
    """
    return g.op("AveragePool1d", input,
                kernel_shape_i=_single(kernel_size),
...
def avg_pool2d(g, input, kernel_size, stride, padding, ceil_mode, count_include_pad):
    ...
    if not stride:
        stride = kernel_size
    #if not count_include_pad:
    #    return _unimplemented("avg_pool2d", "count_include_pad")
    # 由于不需要与其他框架保持格式一致，可以随意增添onnx的nodeparameter的参数
    """
       g.op中的第一个参数是node的名字,然后是输入，之后的参数和顺序都随意，与onnx2caffe工具中的逻辑保持一致就行
    """
...
def avg_pool3d(g, input, kernel_size, stride, padding, ceil_mode, count_include_pad):
    ...
    if not stride:
        stride = kernel_size
    if not count_include_pad:
        return _unimplemented("avg_pool3d", "count_include_pad")
    return g.op("AveragePool", input,
